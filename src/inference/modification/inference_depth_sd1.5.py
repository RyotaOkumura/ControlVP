...
def main(
    ...
    for i, img in enumerate(controlnet_images):
        img.save(
            f"{output_dir}/{timestamp}_depth_gs-{controlnet_guidance_scale}_image-{i}.png"
        )
        print(
            f"individual image {i} saved to {output_dir}/{timestamp}_depth_gs-{controlnet_guidance_scale}_image-{i}.png"
        )

    # グリッド画像を保存
    grid.save(
        f"{output_dir}/{timestamp}_depth_gs-{controlnet_guidance_scale}_comparison.png"
    )
    print(
        f"output image saved to {output_dir}/{timestamp}_depth_gs-{controlnet_guidance_scale}_comparison.png"
    )
...
if __name__ == "__main__":
    ...
    condition_image = depth_estimator(target_image)["depth"]
