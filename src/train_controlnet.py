        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                additional_loss_calculator = AdditionalLossCalculator(
                    config.threshold.distance_from_vanishing_point, config.threshold.edge_magnitude
                )
                
                middle_timesteps = timestep // 2 for timestep in timesteps
                middle_latents = additional_loss_calculator.predict_denoised_latent(
                    noise_scheduler, noisy_latents, model_pred, timesteps, middle_timesteps
                )
                ...
                generated_images = additional_loss_calculator.predict_original_image(
                    vae, noise_scheduler, middle_latents, model_pred_middle, middle_timesteps, None
                )

                additional_loss = additional_loss_calculator.calc_additional_loss(
                    batch["pixel_values"], generated_images
                )
