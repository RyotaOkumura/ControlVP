# code from https://github.com/rishiu/perspective-enhanced-diffusion
import torch
import os
import sys
...
from losses.util import Sobel, find_peaks, get_perp_line
...
def pearson_corr_coef(signal, signal_gt, T):
    numerator = T * torch.sum(torch.mul(signal, signal_gt)) - (
        torch.sum(signal) * torch.sum(signal_gt)
    )
    denom_left = T * torch.sum(torch.square(signal_gt)) - torch.square(
        torch.sum(signal_gt)
    )
    ...
    for vp in vpts:
        ...
        for i in [0, h]:
            for j in [0, w]:
                ...
                angle = torch.arccos(
                    (v / torch.linalg.norm(v))[0]
                    * (-1 if vp[0] > w and 0 <= vp[1] < h else 1)
                ) * torch.sign(v[1])
        ...
        vec = torch.stack(
            ...
            if vp[0] > w and 0 <= vp[1] < h
            else [torch.cos(angles), torch.sin(angles)]
        )
        ...
        x_vals = (
            vp[0] + (vec[0].unsqueeze(1) * ((y - vp[1]) / vec[1].unsqueeze(1)))
        ).long()
        y_vals = (
            vp[1] + (vec[1].unsqueeze(1) * ((x - vp[0]) / vec[0].unsqueeze(1)))
        ).long()
        ...
        y_big_dot_p = torch.abs(
            torch.mul(
                perp_vec[0].unsqueeze(1) * dx[y, x_vals]
                + perp_vec[1].unsqueeze(1) * dy[y, x_vals],
                x_in,
            )
        )
        x_big_dot_p = torch.abs(
            torch.mul(
                perp_vec[0].unsqueeze(1) * dx[y_vals, x]
                + perp_vec[1].unsqueeze(1) * dy[y_vals, x],
                y_in,
            )
        )
        ...
        if False:
            ...
            plt.imshow(
                torch.tile(torch.permute(img.cpu(), (1, 2, 0)), (1, 1, 3)).numpy(),
                cmap="gray",
            )
            for i in range(1000):
                vcpu = vals.cpu().numpy()
