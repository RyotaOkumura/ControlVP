# code from diffusers/examples/controlnet/train_controlnet.py
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--vanishing_point_column",
        type=str,
        default=None,
        help="The column of the dataset containing the vanishing point coordinates.",
    )
    parser.add_argument(
        "--edge_column",
        type=str,
        default="None",
        help="The column of the dataset containing the edges.",
    )
    parser.add_argument(
        "--max_train_samples",
        type=int,
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    if args.vanishing_point_column is None:
        vanishing_point_column = column_names[2]
        logger.info(f"vanishing point column defaulting to {vanishing_point_column}")
    else:
        vanishing_point_column = args.vanishing_point_column
        if vanishing_point_column not in column_names:
            raise ValueError(
                f"`--vanishing_point_column` value '{args.vanishing_point_column}' not found in dataset columns. Dataset columns are: {', '.join(column_names)}"
            )

    if args.edge_column is None:
        edge_column = column_names[3]
        logger.info(f"edges column defaulting to {edge_column}")
    else:
        edge_column = args.edge_column
        if edge_column not in column_names:
            raise ValueError(
                f"`--edge_column` value '{args.edge_column}' not found in dataset columns. Dataset columns are: {', '.join(column_names)}"
            )
    ...
    def preprocess_train(examples):
        ...
        edges = examples[edge_column]
        vanishing_points = examples[vanishing_point_column]

        examples["pixel_values"] = images
        examples["input_ids"] = tokenize_captions(examples)
        examples["vanishing_points"] = vanishing_points
...
def collate_fn(examples):
    ...
    for example in examples:
        condition_image = np.zeros((512, 512, 3), dtype=np.uint8)  # 黒背景

        edges_list = example["edges"]
        if not edges_list:  # エッジがない場合は黒画像のまま
            condition_pil = Image.fromarray(condition_image)
            condition_tensor = conditioning_image_transforms(condition_pil)
            conditioning_pixel_values.append(condition_tensor)
            print("no edges")
            continue
        ...
        condition_pil = Image.fromarray(condition_image)
        # condition_pil.save("condition_image.png")
        condition_tensor = conditioning_image_transforms(condition_pil)
        conditioning_pixel_values.append(condition_tensor)
